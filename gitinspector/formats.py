...
def output_header(runner):
    ...
    if __selected_format__ == "html" or __selected_format__ == "htmlembedded":
        ...
        runner.out.write(html_header.format(title=_("Repository statistics for '{0}'").format(repos_string),
                                            jquery_js=jquery_js,
                                            d3_js=d3_js,
                                            custom_js=custom_js,
                                            style_css=style_css,
        ...
        runner.out.write("{\n\t\"gitinspector\": {\n")
        runner.out.write("\t\t\"version\": \"" + version.__version__ + "\",\n")

        if len(repos) <= 1:
            runner.out.write("\t\t\"repository\": \"" + repos_string + "\",\n")
        else:
            ...
            runner.out.write(repos_json[:-2] + " ],\n")

        runner.out.write("\t\t\"report_date\": \"" + time.strftime("%Y/%m/%d") + "\",\n")

    elif __selected_format__ == "xml":
        runner.out.write("<gitinspector>\n")
        runner.out.write("\t<version>" + version.__version__ + "</version>\n")

        if len(repos) <= 1:
            runner.out.write("\t<repository>" + repos_string + "</repository>\n")
        else:
            runner.out.write("\t<repositories>\n")

            for repo in repos:
                runner.out.write("\t\t<repository>" + repo.name + "</repository>\n")

            runner.out.write("\t</repositories>\n")

        runner.out.write("\t<report-date>" + time.strftime("%Y/%m/%d") + "</report-date>\n")
    else:
        ...
        runner.out.write(textwrap.fill(repos_text, width=terminal.get_size()[0]) + "\n")
